...
Функция ПолучитьОписание(opCode, Operand, Адрес)
	...
	ИначеЕсли opCode = 45 Тогда // "Завершение блока"
		...
		//	Если ЭтоПроцедура Тогда
		//		
		//		Возврат УстановитьЗначение("Завершение блока", "КонецПроцедуры", 0);
		//		
		//	ИначеЕсли ЭтоФункция Тогда
		//		
		//		Возврат УстановитьЗначение("Завершение блока", "КонецФункции", 0); 
		//		
		//	КонецЕсли;
		//	
		//КонецЕсли;
...
Процедура ЗагрузитьДанныеФункцииПроцедуры(Наименование)   
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЭтоФункция Тогда
			...
			Объект.Text = "Функция " + СокрЛП(Наименование) + "(" + СокрЛП(ПараметрыПроцедурыФункции) + ")" + ?(Экспортная, " Экспорт", "") + Символы.ПС;  
			
		ИначеЕсли ЭтоПроцедура Тогда 
			
			Объект.Text = "Процедура " + СокрЛП(Наименование) + "(" + СокрЛП(ПараметрыПроцедурыФункции) + ")" + ?(Экспортная, " Экспорт", "") + Символы.ПС;   
...
Процедура ПроцедурыФункцииОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	Для А = 0 По Элементы.ПроцедурыФункции.СписокВыбора.Количество() - 1 Цикл 
		...
		Если Элементы.ПроцедурыФункции.СписокВыбора.Получить(А).Значение = ВыбранноеЗначение Тогда 
			...
			ПроцедурыФункции = Элементы.ПроцедурыФункции.СписокВыбора.Получить(А).Представление; 
