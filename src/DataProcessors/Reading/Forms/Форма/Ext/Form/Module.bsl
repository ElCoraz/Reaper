...
Процедура ОбновитьДанные()
	...
	Пока Выборка.Следующий() Цикл
		...
		Параметры	 = Выборка.ПараметрыПроцедурыФункции;
...
Функция УстановитьЗначение(Описание, ОперацияОпКода, КоличествоПараметров) 
	...
	Если КоличествоПараметров > 0 Тогда
		...
		Массив = Новый Массив(КоличествоПараметров);
		
		А = КоличествоПараметров - 1;
		
		Пока А > 0 Цикл 
			
			Массив[А] = СокрЛП(СтрПолучитьСтроку(Объект.Text, СтрЧислоСтрок(Объект.Text) - А)); 
			
			А = А - 1;
			
		КонецЦикла;
		
		Объект.Text = Объект.Text + СтрШаблон(ОперацияОпКода, Массив) + Символы.ПС; 
