		...
		Если КоличествоПараметров > 0 Тогда
			...
			Для А = 0 По Массив.Количество() - 1 Цикл 
				...
				Если ЗначениеЗаполнено(Строка(Массив[А])) Тогда  
					...
					Массив[А] = СтрЗаменить(Строка(Массив[А]), """", "_!SSS!_");
					
					ПараметрыВыполнения = ПараметрыВыполнения + "" + """" + Строка(Массив[А]) + """" + ", ";
					
				Иначе 
					
					ПараметрыВыполнения = ПараметрыВыполнения + """_!SSS!_"", ";  
			...
			Временная = СтрЗаменить(Временная, "_!SSS!_", """");
...
//*****************************************************************************************
&НаСервере
Функция ЭтоЧисло(Переменная) 
	
	Попытка

		Число = Число(Лев(Переменная, 1));   
		
		Возврат Число >= 0 И Число <= 9;			
...
Функция ПолучитьОписание(opCode, Operand, Адрес)
	...
	ИначеЕсли opCode = 2 Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если УстановитьПрефиксДляПеременнойЧисла Тогда
				
				Если ЭтоЧисло(Переменная) Тогда 
					
					Переменная = "_" + Переменная;
	...
	ИначеЕсли opCode = 4 Тогда 
		...
		Если Выборка.Следующий() Тогда 
			...
			Если СокрЛП(Выборка.Вид) = "S" Тогда 
				
				Объект.Text = Объект.Text + " _!SSS!_" + Строка(Переменная) + "_!SSS!_" + Символы.ПС; 
	...
	ИначеЕсли opCode = 10 Тогда // "Загрузить на стек пустое значение"
		...
		Объект.Text = Объект.Text + "_!SSS!__!SSS!_" + Символы.ПС;
	...
	ИначеЕсли opCode = 15 Тогда // "Получить из выборки очередной элемент коллекции. Этот элемент попадает в переменную, которая была на вершине стека, а в стек вместо этой переменной помещается результат успешности (Истина - получили очередной элемент, Ложь - нет)"
		...
		Возврат УстановитьЗначение("Получить из выборки очередной элемент коллекции", "Пока %1.Следующий() Цикл", 1);
	...
	ИначеЕсли opCode = 16 Тогда 
		...
		ИначеЕсли Объект.Приоритет = 5 Тогда  
			
			УстановитьПриоритет(0);

			Возврат УстановитьЗначение("=", "%1 = %2 %3", 3); 
	...
	ИначеЕсли opCode = 24 Тогда // "Операция +" 
		...
		Если СтрНайти(СокрЛП(СтрПолучитьСтроку(Объект.Text, Количество - 1)), "/") > 0 
			ИЛИ СтрНайти(СокрЛП(СтрПолучитьСтроку(Объект.Text, Количество - 1)), "*") > 0 Тогда 
	...
	ИначеЕсли opCode = 26 Тогда // "Операция *"
		...
		Если СтрНайти(СокрЛП(СтрПолучитьСтроку(Объект.Text, Количество - 1)), "-") > 0 
			ИЛИ СтрНайти(СокрЛП(СтрПолучитьСтроку(Объект.Text, Количество - 1)), "/") > 0 
			ИЛИ СтрНайти(СокрЛП(СтрПолучитьСтроку(Объект.Text, Количество - 1)), "+") > 0 Тогда 
	...
	ИначеЕсли opCode = 29 Тогда // "Операция НЕ"
		...
		Если Объект.Приоритет = 1 Тогда 
			
			УстановитьПриоритет(1);
			
			Возврат УстановитьЗначение("Операция НЕ", "НЕ %1", 1);
	...
	ИначеЕсли opCode = 30 Тогда // "Операция И. Если операнд на вершине стека Ложь, то происходит переход по адресу, указаному во втором параметре, т.е. пропуск вычисления второго операнда, т.к. от него уже ничего не зависит. Из стека при этом значение не удаляется (результатом операции является Ложь). Иначе, если на вершине стека Истина, из стека удаляется значение и перехода не происходит, т.е. вычисляется второй операнд, результат которого полностью определяет результат всей операции)"
		...
		Если Объект.Приоритет = 2 Тогда    
			...
			УстановитьПриоритет(2);
			
			Возврат УстановитьЗначение("Операция И", "%1 И", 1);
	...
	ИначеЕсли opCode = 31 Тогда // "Операция ИЛИ. Если операнд на вершине стека Истина, то происходит переход по адресу, указаному во втором параметре, т.е. пропуск вычисления второго операнда, т.к. от него уже ничего не зависит. Из стека при этом значение не удаляется (результатом операции является Истина). Иначе, если на вершине стека Ложь, из стека удаляется значение и перехода не происходит, т.е. вычисляется второй операнд, результат которого полностью определяет результат всей операции)"
		...
		Если Объект.Приоритет = 2 Тогда    
			...
			Возврат УстановитьЗначение("Операция ИЛИ", "%1 ИЛИ", 1);
	...
	ИначеЕсли opCode = 32 Тогда // "Операция <"
		...
		Если Объект.Приоритет = 2 Тогда    
			...
			Возврат УстановитьЗначение("Операция <", "_### %1 < %2 ###_", 2);
	...
	ИначеЕсли opCode = 39 Тогда // "Переход на заданный шаг"
		...
		Кол = СтрЧислоВхождений(Объект.Text, "###_");
		...
		Если Кол > 0 Тогда
			...
			Для А = 1 По СтрЧислоСтрок(Объект.Text) Цикл
				
				ТекущаяСтрока = СтрПолучитьСтроку(Объект.Text, А);
				
				Если Первая И Найти(ТекущаяСтрока, "_###") > 0 Тогда
					
					ТекущаяСтрока = СтрЗаменить(ТекущаяСтрока, "_###", "Если"); 
					
					Первая = Ложь;
					
				Иначе
					
					ТекущаяСтрока = СтрЗаменить(ТекущаяСтрока, "_###", ""); 
					
				КонецЕсли;
				
				Если Б = Кол Тогда
					
					ТекущаяСтрока = СтрЗаменить(ТекущаяСтрока, "###_", "Тогда"); 
	...
	ИначеЕсли opCode = 44 Тогда // "Попытка"   
		...
		УстановкаПопытка = Истина;
		
		Возврат УстановитьЗначение("Попытка", "Попытка", 0);
		
	ИначеЕсли opCode = 45 Тогда // "Завершение блока"
		
		//Добавить вложенность вывозовов
		
		Если УстановкаПопытка Тогда  
			
			Если Operand > 0 Тогда 
				
				Возврат УстановитьЗначение("Исключение", "Исключение", 0);
...
//*****************************************************************************************  
&НаСервере
Процедура ЗагрузитьДанныеФункцииПроцедуры(Адрес, Очистить = Ложь)   
...
//*****************************************************************************************  
&НаСервере
Функция ПолучитьТекстФункцииПроцедуры()
	...
	Запрос.УстановитьПараметр("Файл",	 Объект.Файл);
	Запрос.УстановитьПараметр("Адрес",	 Число(ПроцедурыФункции));
...
Процедура ПроцедурыФункцииОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	Объект.Text = ПолучитьТекстФункцииПроцедуры();   

	ЗагрузитьДанныеФункцииПроцедуры(ПроцедурыФункции, Объект.Text = ""); 
...
Процедура ФайлПриИзмененииНаСервере()
	...
	НайденнаяСтрока = Объект.Проект.Файлы.Найти(Объект.Файл, "Наименование");

	Если НайденнаяСтрока <> Неопределено Тогда
		
		Объект.Text = НайденнаяСтрока.Code;
...
Процедура TextОткрытие(Элемент, СтандартнаяОбработка) 
	...
	Форма = ПолучитьФорму("ОбщаяФорма.ПросмотраКода", Новый Структура("text", Объект.Text));
...
//*****************************************************************************************  
&НаКлиенте
Процедура TextОчистка(Элемент, СтандартнаяОбработка)  
	
	СтандартнаяОбработка = Ложь;
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект, Параметры), "Очистить текст модуля?", РежимДиалогаВопрос.ДаНет, 0);
...
Процедура SaveНаСервере()
	...
	ОбъектПроекта = Объект.Проект.ПолучитьОбъект();

	НайденныеСтроки = ОбъектПроекта.ФункцииПроцедуры.НайтиСтроки(Новый Структура("Файл, Адрес", Объект.Файл, Число(ПроцедурыФункции)));

	Если НайденныеСтроки.Количество() > 0 Тогда  

		НайденныеСтроки[0].Текст = Объект.Text;
...
Процедура Save(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаСохранение", ЭтотОбъект, Параметры), "Сохранить текст модуля?", РежимДиалогаВопрос.ДаНет, 0);
